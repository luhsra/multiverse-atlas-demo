...
\begin{tikzpicture}[
  ...
  \csdef{unit_}#1{\node[T=#1,time unit=0.5,calc] (X#1){};
    \begin{pgfonlayer}{foreground}
      \draw[migratecolor,ultra thick] (X#1.north east) -- (X#1.south east);
    \end{pgfonlayer}
  }
  \csdef{unitb}#1{\node[T=#1,time unit=0.5,block={badbee!50!black}] {};}

  \colorlet{migratecolor}{srared}
  ...
  \node[T=4,Label=3] (t4-label) {Patcher};


  \draw[thick,->] (t4-label.south east) ++(down:1em) coordinate(@) --  ++(east:6.5cm) node[right]{\textit{t}};
  \coordinate (@) at (@-| t4-label.east);
  \foreach \x in {0,...,12} {
    ...
    \timeline{3}{----f}
    \timeline{4}{--}
    \colorlet{calccolor}{luhblue}\node[T=4,time unit=1.5,calc] {apply};
    \timeline{4}{-------}
        


    \marker[luhblue,ultra thick]{2}{patch\\$f\rightarrow f'$};
    \marker[badbee!70!black,ultra thick]{5}{start\\migration};


    \colorlet{migratecolor}{safegreen}
    \timeline{1}{-f==_}
    \timeline{2}{=_-}
    \timeline{3}{_-g}
    \marker[safegreen,ultra thick]{10}{done};
    ...
    \draw[decorate,decoration={brace,amplitude=5pt}]%
       (tick5 |- t1-label.north)%
        -- node[above,yshift=5pt,font=\scriptsize,xshift=-1ex] {migration phase\mathstrut}%
        (tick10 |- t1-label.north);
